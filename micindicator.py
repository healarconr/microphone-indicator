...
gi.require_version('Keybinder', '3.0')  # nopep8

from gi.repository import AppIndicator3 as appindicator  # nopep8
from gi.repository import GdkPixbuf, Gtk, Keybinder, Notify  # nopep8
...
class Indicator():
    ...
    def __init__(self):
        self.indicator = appindicator.Indicator.new(
            APPINDICATOR_ID, self.get_current_state_icon(), appindicator.IndicatorCategory.SYSTEM_SERVICES)
        ...
        Keybinder.bind(keystr, self.callback_toggle_mic,
                       "keystring %s (user data)" % keystr)
    ...
    def show_about_dialog(self, _):
        ...
        aboutdialog.set_comments(
            "AppIndicator to mute and show microphone mute status.")
        aboutdialog.set_logo(GdkPixbuf.Pixbuf.new_from_file(
            self.get_resource("icon.svg")))
        aboutdialog.set_authors(authors)
        aboutdialog.set_documenters(documenters)
        aboutdialog.set_website(
            "https://github.com/sidneibjunior/microphone-indicator")
    ...
    def get_current_mic_state(self):
        ps = subprocess.Popen(("amixer", "get", "Capture"),
                              stdout=subprocess.PIPE,
                              encoding="utf-8")
        output = subprocess.check_output(
            ('egrep', '-o', '\[o.+\]', '-m', '1'), stdin=ps.stdout, encoding="utf-8")
        ps.wait()
        return output.strip()
    ...
    def build_menu(self):
        ...
        self.item_toggle = Gtk.MenuItem(label='Toggle Microphone')
        self.item_toggle.connect('activate', self.toggle_mic)
        menu.append(self.item_toggle)

        self.item_about = Gtk.MenuItem(label='About')
    ...
    def update_mic_state(self):
        ...
        self.indicator.set_icon_full(icon_name=self.get_current_state_icon(),
                                     icon_desc="Microphone indicator")
